...
public:
	int canBeTypedWords(string text, string brokenLetters) {
        ...
        while(getline(ss, wordToken, ' ')) {
        	if(wordToken != "") words.push_back(wordToken);
        }
        set<char> bls(brokenLetters.begin(), brokenLetters.end());

       	int count = 0;
       	for(auto word : words) {
       		bool couldBeTyped = true;
       		for(auto ch : word) {
       			if(bls.find(ch) != bls.end()) {
       				couldBeTyped = false;
       				continue;
       			}
       		}
       		if(couldBeTyped) count++;
       	}
       	return count;
    }
};
...
void solve() {
	...
	string text;
	getline(cin, text);
	string brokenLetters; cin >> brokenLetters;	
	
	Solution *soln = new Solution();
	cout << soln->canBeTypedWords(text, brokenLetters);
