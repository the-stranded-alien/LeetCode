// Assume you are an awesome parent and want to give your children some cookies.
// But, you should give each child at most one cookie.
// Each child i has a greed factor g[i], which is the minimum size of
// a cookie that the child will be content with; and each cookie j has
// a size s[j]. If s[j] >= g[i], we can assign the cookie j to the child i,
// and the child i will be content. Your goal is to maximize the number of your
// content children and output the maximum number.

#include<iostream>
#include<vector>
using namespace std;

class Solution {
public:
    int findContentChildren(vector<int>& g, vector<int>& s) {
        if(s.size() == 0) return 0;
        sort(g.begin(), g.end());
        sort(s.begin(), s.end());
        int greedIndex = 0;
        int cookieSizeIndex = 0;
        int childrenContent = 0;
        while (greedIndex < g.size() && cookieSizeIndex < s.size()) {
            if(s[cookieSizeIndex] >= g[greedIndex]) {
                childrenContent++;
                greedIndex++;
            }
            cookieSizeIndex++;
        }
        return childrenContent;
    }
};

int main() {
    int n; cin >> n;
    vector<int> g(n);
    for(int i = 0; i < n; i++) cin >> g[i];
    int m; cin >> m;
    vector<int> s(m);
    for(int i = 0; i < m; i++) cin >> s[i];
    Solution *soln = new Solution();
    cout << soln->findContentChildren(g, s) << endl;
    return 0;
}