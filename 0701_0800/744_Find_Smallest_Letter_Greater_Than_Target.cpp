...
// array that is larger than target. Note that the letters wrap around.
// For example, if target == 'z' and letters == ['a', 'b'], the answer is 'a'.

class Solution {
public:
    char nextGreatestLetter(vector<char>& letters, char target) {
        for(char cl : letters) if(cl > target) return cl;
        return letters[0];
    }
};
...
void solve() {
	...
	char ngl = soln->nextGreatestLetter(letters, target);
	cout << ngl << nl;
	return;
}
