...
// between them. The score of a pair (i < j) of sightseeing spots is values[i]
...
public:
    int maxScoreSightseeingPair(vector<int>& values) {
        ...
        for(int i = 1; i < values.size(); i++) {
        	...
        	maxSoFar = max(values[i] + i, maxSoFar);
        	cout << maxScore << " -- " << maxSoFar << nl; 
        }
        return maxScore;
    }
};
...
void solve() {
	...
	int maxScore = soln->maxScoreSightseeingPair(values);
	cout << maxScore << nl;
	return;	
}
