...
public:
	vector<string> commonChars(vector<string>& words) {
        ...
        for(auto word : words) {
        	vector<int> curWordFreq(26, 0);
        	for(auto ch : word) curWordFreq[ch - 'a']++;
        	for(int i = 0; i < 26; i++) {
        		minFreq[i] = min(minFreq[i], curWordFreq[i]);
        	}
        }
        vector<string> cc;
        for(int i = 0; i < 26; i++) {
