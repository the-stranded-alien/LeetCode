...
public:
    string destCity(vector<vector<string>>& paths) {
        ...
        for(int i = 0; i < paths.size(); i++) graph[paths[i][0]] = paths[i][1];
...
void solve() {
	// Main Code Goes Here !!
	int path; cin >> path;
	vector< vector<string> > paths(path, vector<string>(2, " "));
	for(int i = 0; i < path; i++) cin >> paths[i][0] >> paths[i][1];
	Solution *soln = new Solution();
	string dest = soln->destCity(paths);
	cout << dest << nl;
	return;
}
