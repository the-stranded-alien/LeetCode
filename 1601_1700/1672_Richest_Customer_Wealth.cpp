...
public:
    int maximumWealth(vector<vector<int>>& accounts) {
        ...
        for(int pItr = 0; pItr < m; pItr++) {
        	int curPerson = 0;
        	for(int aItr = 0; aItr < n; aItr++) curPerson += accounts[pItr][aItr];
        	if(curPerson > maxWealth) maxWealth = curPerson;
        }
        return maxWealth; 
    }
};
...
void solve() {
	...
	int n;
	cin >> m >> n;
	vvi accounts(m, vector<int>(n, 0)); 
	for(int i = 0; i < m; i++) {
		for(int j = 0; j < n; j++) cin >> accounts[i][j];
	}
	Solution *soln = new Solution();
	int maxWealth = soln->maximumWealth(accounts);
	cout << maxWealth << nl;
	return;
}
